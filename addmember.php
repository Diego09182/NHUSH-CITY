<?php
  ...
  if (mysqli_num_rows($result) != 0)
  {
    ...
    echo "alert('您所註冊的帳號已經有人使用，請使用其它帳號');";
    echo "history.back();";
    echo "</script>";
  }
  //如果帳號沒人使用
  else
  {
    ...
    //執行 SQL 命令，新增此帳號
    $sql = "INSERT INTO users (account,password,name,year,month,day,student_ID,cellphone,interest,email,url,comment) VALUES 
			('$account','$password_hash','$name','$year','$month','$day','$student_ID','$cellphone','$interest','$email','$url','$comment')";
    $result = execute_sql($link, "member", $sql);
  }
